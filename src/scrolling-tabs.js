...
;(function ($) { // eslint-disable-line no-extra-semi
    ...
    const scrollingTabsWrapperTemplate = [
    ...
    (function (p) {
        ...
        p.handleClickOnRightScrollArrow = function () {
            const evh = this;
            const stc = evh.stc;
            const scrollMovement = stc.scrollMovement;
        ...
        p.handleWindowResize = function () {
            const evh = this;
            const stc = evh.stc;
            const newWinWidth = stc.$win.width();
    ...
    (function (p) {
        ...
        p.moveTabContentOutsideScrollContainer = function (options) {
            const ehd = this;
            const stc = ehd.stc;
            const $tabsContainer = stc.$tabsContainer;
            const tabContentCloneCssClass = 'scrtabs-tab-content-clone';
            const $tabContentInMovableContainer = $tabsContainer.find('.scrtabs-tabs-movable-container .tab-content');
            ...
            // create new clone
            const $newTcClone = $tabContentInMovableContainer
                .clone()
                .addClass(tabContentCloneCssClass);

            // get the current clone, if it exists
            const $currTcClone = $tabsContainer.find(`.${tabContentCloneCssClass}`);
        ...
        p.refreshAllElementSizes = function () {
            const ehd = this;
            const stc = ehd.stc;
            const smv = stc.scrollMovement;
            const scrollArrowsWereVisible = stc.scrollArrowsVisible;
            const actionsTaken = {
        ...
        p.setEventListeners = function () {
            const ehd = this;
            const stc = ehd.stc;
            const evh = stc.eventHandlers; // eventHandlers
        ...
        p.setFixedContainerWidth = function () {
            const ehd = this;
            const stc = ehd.stc;
            const tabsContainerRect = stc.$tabsContainer.get(0).getBoundingClientRect();
        ...
        p.setMovableContainerWidth = function () {
            const ehd = this;
            const stc = ehd.stc;
            const $tabLi = stc.$tabsUl.find('li');
            const fixedContainerHeight = stc.$fixedContainer.height();
            let maxWidthExpansions = 10; // prevent infinite loop
        ...
        p.setScrollArrowVisibility = function () {
            const ehd = this;
            const stc = ehd.stc;
            const shouldBeVisible = stc.movableContainerWidth > stc.fixedContainerWidth;
    ...
    (function (p) {
        ...
        p.continueScrollRight = function (minPos) {
            ...
            stc.$timeout(function () {
                if (stc.$rightScrollArrow.data(CONSTANTS.DATA_KEY_IS_MOUSEDOWN)
        ...
        p.getMinPos = function () {
            ...
            return stc.scrollArrowsVisible ?
                (stc.fixedContainerWidth - stc.movableContainerWidth - stc.scrollArrowsCombinedWidth)
                : 0;
        };
        ...
        p.scrollToActiveTab = function () {
            ...
            const $activeTab = stc.$tabsUl.find('li.active');

            if (!$activeTab.length) {
                return;
            }

            const activeTabLeftPos = $activeTab.offset().left;
            const activeTabWidth = $activeTab.outerWidth();
            const activeTabRightPos = activeTabLeftPos + activeTabWidth
                + parseInt(stc.scrollingTabsActiveOffset || 0, 10);

            const rightArrowLeftPos = stc.$rightScrollArrow.offset().left;
            const leftArrowRightPos = stc.$leftScrollArrow.outerWidth(); // its leftpos is 0
            const visibleAreaLeftPos = leftArrowRightPos;
            const visibleAreaRightPos = rightArrowLeftPos;

            if (activeTabLeftPos < visibleAreaLeftPos) {
                offset = activeTabLeftPos - visibleAreaLeftPos;
                stc.movableContainerLeftPos -= offset;
        ...
        p.setMovableContainerLeftPosToTabEdge = function (scrollArrowClicked) {
            const smv = this;
            const stc = smv.stc;
            const offscreenWidth = -stc.movableContainerLeftPos;
            let totalTabWidth = 0;
        ...
        p.slideMovableContainerToLeftPos = function () {
            ...
            stc.movableContainerLeftPos /= 1;
            const leftVal = smv.getMovableContainerCssLeftVal();

            stc.$movableContainer.stop().animate({ left: leftVal }, 'slow', function __slideAnimComplete() {
                const newMinPos = smv.getMinPos();
    ...
    (function (p) {
        p.handleTriggeredRefresh = function () {
            const stc = this;
            const scrollMovement = stc.scrollMovement;
            const actionsTaken = stc.elementsHandler.refreshAllElementSizes(true);
        ...
        p.initTabs = function (options) {
            const stc = this;
            const elementsHandler = stc.elementsHandler;
            const scrollMovement = stc.scrollMovement;
            ...
            stc.$timeout(function __initTabsAfterTimeout() {
                const actionsTaken = elementsHandler.refreshAllElementSizes();
        ...
        p.handleArrowsClassNames = function (edge) {
            const stc = this;
            const edgeClassName = 'scrtabs-tab-scroll-arrow--is-on-edge';
            const arrows = {
    ...
    function scrollingTabsDirective($timeout, $sce) {
        ...
        return {
            ...
            link: function (scope, element, attrs) {
                const scrollingTabsControl = new ScrollingTabsControl(element, $timeout);
                const scrollToTabEdge = attrs.scrollToTabEdge && attrs.scrollToTabEdge.toLowerCase() === 'true';
                ...
                element.on('click.scrollingTabs', '.nav-tabs > li', function __handleClickOnTab(e) {
                    const clickedTabElData = $(this).data();
                ...
                scope.$watch('watchTabs', function (latestTabsArray) {
                    let $activeTabLi;
                    let activeIndex;
    ...
    function scrollingTabsWrapperDirective($timeout) {
        ...
        return {
            ...
            link: function (scope, element, attrs) {
                ...
                const isWrappingAngularUIBTabset = element.find('uib-tabset').length > 0;
                const isWrappingAngularUITabset = isWrappingAngularUIBTabset || element.find('tabset, .scrtabs-tabs-movable-container div > ul.nav').length > 0;
                const scrollToTabEdge = attrs.scrollToTabEdge && attrs.scrollToTabEdge.toLowerCase() === 'true';
                const scrtc = {
                ...
                scope.$watch(attrs.watchTabs, function (latestTabsArray) {
                ...
                function initTabsAsWrapperWatchingTabs(options) {
                    scrollingTabsControl.initTabs({
                        ...
                        isWatchingTabs: true,
                        isWrapperDirective: true,
                        isWrappingAngularUITabset,
                        scrollToTabEdge,
                    });
                }

                function watchForTabDomChanges() {
                    const $navTabs = element.find('ul.nav-tabs');
                    let currActiveTabIdx;
                    let showingActiveTabIdx;
